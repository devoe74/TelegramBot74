...
public class GraphsPanel extends JPanel {
    ...
    public ImageIcon calcDoughnut() throws IOException {
        ...
        String use = "0, 0, 0";
        if (Arrays.stream(Bot74.selectedChoicesUses).sum() > 0){
            use = decimalFormat.format((((double) Bot74.selectedChoicesUses[0]) / ((double) Arrays.stream(Bot74.selectedChoicesUses).sum())) * 100)
                    + "," + decimalFormat.format((((double) Bot74.selectedChoicesUses[1]) / ((double) Arrays.stream(Bot74.selectedChoicesUses).sum())) * 100)
                    + "," + decimalFormat.format((((double) Bot74.selectedChoicesUses[2]) / ((double) Arrays.stream(Bot74.selectedChoicesUses).sum())) * 100);
        }
        chart.setWidth(500);
        chart.setHeight(400);
        chart.setVersion("2.9.4");
        chart.setConfig("{" +
                ...
                "        labels: [" +
                "          {" +
                "            text: '" + Arrays.stream(Bot74.selectedChoicesUses).sum() + "'," +
                "            font: {" +
                "              size: 20," +
                "              weight: 'bold'," +
                "            }," +
    ...
    public void updateMassagesAndUsersIn1234minutes(){
        new Thread(()->{
            while (true){
                this.massagesAndUsersIn1234minutes[0][0] = Bot74.usersAndMassagesInLastMinute[0];
                this.massagesAndUsersIn1234minutes[0][1] = Bot74.usersAndMassagesInLastMinute[1];

                if (LocalDateTime.now().getSecond() == SECONDS_IN_START_MOMENT){
                    moveMassagesAndUsersIn1234minutes();
                    Bot74.usersAndMassagesInLastMinute[0] = 0;
                    Bot74.usersAndMassagesInLastMinute[1] = 0;
                }
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e) {
